  ...
  return (
    <div style={{ paddingBottom: 100, background: "#f9fafb", minHeight: "100%" }}>
      ...
      <div style={{ padding: "16px", background: "#fff", display: "flex", alignItems: "center", gap: 12, borderBottom: "1px solid #e5e7eb", position: "sticky", top: 0, zIndex: 10 }}>
        <button onClick={() => setActiveTab("products")} style={{ background: "none", border: "none", fontSize: 24, color: "#111827", cursor: "pointer", padding: 0 }}>←</button>
      ...
      <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ background: "#fff", borderRadius: 16, padding: 16, border: "1px solid #e5e7eb", boxShadow: "0 2px 4px rgba(0,0,0,0.02)" }}>
           <h3 style={{ margin: "0 0 12px 0", fontSize: 15, fontWeight: 700, color: "#111827", display: "flex", alignItems: "center", gap: 8 }}>📍 Delivery Address</h3>
           {addresses.length === 0 ? (
        ...
        {/* ⏱️ SHIFT SELECTION */}
        <div style={{ background: "#fff", borderRadius: 16, padding: 16, border: "1px solid #e5e7eb", boxShadow: "0 2px 4px rgba(0,0,0,0.02)" }}>
           <h3 style={{ margin: "0 0 12px 0", fontSize: 15, fontWeight: 700, color: "#111827", display: "flex", alignItems: "center", gap: 8 }}>⏱️ Select Shift</h3>
               ...
               <div key={item.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                 <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                   <div style={{ width: 48, height: 48, background: "#f8fafc", borderRadius: 8, padding: 4, display: "flex", alignItems: "center", justifyContent: "center" }}>
                     {item.imageUrl ? <img src={item.imageUrl} alt={item.name} style={{ width: "100%", height: "100%", objectFit: "contain", mixBlendMode: "darken" }} /> : "📦"}
                   </div>
                   <div>
                     <div style={{ fontSize: 13, fontWeight: 700, color: "#111827", marginBottom: 2 }}>{item.name}</div>
                     <div style={{ fontSize: 11, color: "#6b7280", fontWeight: 600 }}>{item.unit} • ₹{item.price}</div>
                   </div>
                 </div>
